...
class TestApis(TestCase):
    def test_api(self):
        response = self.client.get('/api/v1/echo?msg=right')
        result = json.loads(response.content)
        self.assertEqual(result.get("msg"), "right")

    def test_api_without_parameter(self):
        response = self.client.get('/api/v1/echo?message=right')
        result = json.loads(response.content)
        self.assertEqual(result.get("msg"), None)
